...
PATH should point to the 'text-en' directory of the pgf distribution
]])
   return
end
...
for f in lfs.dir(arg[1]) do
  if f:match("%.tex$") then
    local s = io.open(arg[1] .. f):read"all"
...
for sig, text in pairs(items.command) do
  ...
  commands[csname].documentation = "texmf:doc/generic/pgf/pgfmanual.pdf#pgf.back/" .. csname
...
for sig, text in pairs(items.key) do
  ...
  res.documentation = "texmf:doc/generic/pgf/pgfmanual.pdf#pgf." .. key:gsub(" ", ":")
...
for _, k in ipairs(tikzpathcommands) do
  commands[k].arguments = {{meta="specification", delimiters = {"",";"}}}
  commands[k].action = 'tikzpath'
  if k ~= "path" and util.nested_get(tikz, "keys", "tikz", k, "details") and commands[k].details then
    commands[k].summary = clean(commands[k].details)
    commands[k].details = tikz.keys.tikz[k].details
  end
end
...
documentation = {
  {
    summary = "PGF Manual",
    uri = "texmf:doc/generic/pgf/pgfmanual.pdf"
  }
}

f = io.open("tikz.tags", "w")
f:write("comments = ", inspect(comments), "\n")
f:write("ctan_package = \"tikz\"\n")
f:write("documentation = ", inspect(documentation), "\n")
f:write("commands = ", inspect(tikz.commands), "\n")
f:write("keys = ", inspect(tikz.keys), "\n")

f = io.open("pgf.tags", "w")
f:write("comments = ", inspect(comments), "\n")
f:write("ctan_package = \"pgf\"\n")
f:write("documentation = ", inspect(documentation), "\n")
f:write("commands = ", inspect(pgf.commands), "\n")
f:write("keys = ", inspect(pgf.keys), "\n")
